...
На Windows версия 0.1.13 работает внутри обычного ZCode и не меняет файлы приложения. После первой установки один раз открой новую задачу, затем выбери **Settings > General > Language > Русский** или **Профиль > Язык > Русский**. Перезапуск ZCode не нужен.

После первого `SessionStart` runtime отслеживает состояние плагина. При выключении русский язык и перевод исчезают, при включении возвращаются, а обновления применяются перезагрузкой только renderer.
