...
if ($SelfTest) {
    ...
    try {
        ...
        @{ plugins = @(@{ id = $PluginId; version = "0.1.21"; installPath = $testRoot }) } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $testInstalled -Encoding UTF8
        ...
        if ($testPlugin.version -ne "0.1.21") { throw "Installed plugin version was not detected." }
        ...
        $fingerprintA = Get-StateFingerprint -Payload ([ordered]@{ version = "0.1.21"; status = "active"; updatedAt = "first" })
        $fingerprintB = Get-StateFingerprint -Payload ([ordered]@{ version = "0.1.21"; status = "active"; updatedAt = "second" })
