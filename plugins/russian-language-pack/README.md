...
Плагин A.I.R ZCode Plugins версии 0.1.13 добавляет русский язык и перевод интерфейса в обычный ZCode Desktop 3.3.4 на Windows 10 и Windows 11.
...
* После перезагрузки Windows runtime восстанавливается при новой задаче или первом запросе.
...
`SessionStart` запускает фоновый runtime. Он подключается к обычному ZCode, внедряет перевод только в память renderer и следит за состоянием плагина. `UserPromptSubmit` восстанавливает runtime при необходимости и добавляет русскоязычный контекст к запросу.
...
Поддерживаются только Windows 10, Windows 11 и ZCode Desktop 3.3.4.
